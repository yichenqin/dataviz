---
title: "Ch4 Visualization Basics"
...
We mostly use the scatter plot as a running example and introduce different aspects of data visualization and the logic behind it.
...
library(RColorBrewer) # for color pallete
...
Obviously, data visualization plays important roles in this flowchart.
...
For detailed introduction to the grammar of graphics, please see @Wickham2010 (<http://vita.had.co.nz/papers/layered-grammar.pdf>).

According to the grammer of graphics, there are in total seven layers/components in a data visualization, listed below.

| Layer              | Priority | Description                                                                                                                                                                                         | Example                                                                                                                            |
|-----------------|-----------------|--------------------|--------------------|
| Data               | Required | Data set being plotted                                                                                                                                                                              | A n-by-p table with variables in the columns and observations in the rows.                                                         |
| Geometries         | Required | The visualization type or the visual elements used in the plot                                                                                                                                      | Scatter plot, bar plot, histogram, boxplot, and etc.                                                                               |
| Aesthetic mappings | Required | The aesthetic dimensions or visual elements onto which the variables are mapped, e.g., height goes to x-axis, weight goes to y-axis, gender goes to symbol color, age goes to symbol size, and etc. | x-axis, y-axis, symbol color, symbol size, symbol shape, transparency level, line width, line type, filled color, labels, and etc. |
| Statistics         | Optional | Summary statistics to be visualized                                                                                                                                                                 | Fitted linear/nonlinear regression regressions, confidence intervals, counting, binning, etc.                                      |
| Coordinates/scale  | Optional | The space the data is plotted, e.g., coordination systems. The scale of each aesthetics dimensions, e.g, axis range, transformations, tick positions.                                               | Cartesian, fixed, polar, limits, transformation, range, tick, and etc.                                                             |
| Facets             | Optional | Plotting small multiple visualizations together for comparison                                                                                                                                      | Side by side, stacked, columns and rows.                                                                                           |
| Themes             | Optional | All non-essential formatting                                                                                                                                                                        | Background, foreground, grid, label, font, color, bold, rotated text.                                                              |

When creating a new visualization, we first need to specify the data set to be visualized, i.e., the data layer.
Then we specify the visualization type, i.e., scatterplot or barplot.
...
For example, variable x1 on the x-axis and variable x2 on the y-axis and etc.

These three layers decide the most important aspect of the visualization, and therefore are required for any visualizations.
Once these three layers are specified, the analysts can work on the remaining layers of visualization, such as some summary statistics, coordinate systems, scales, facets, and themes.
Basically, the grammar of graphics decomposes each plotting task into several distinct layers of grammatical elements.

Throughout this book, we mostly use `ggplot()` function generate data visualizations, because the syntax in ggplot() function embodies the grammar of graphics.
...
In addition to `ggplot()` function, many other visualization tools such as Tableau and Power BI also embodies the same logic.
...
A complete list of functions in ggplot can be found in its official website[^1].
In this book, we do not attempt to exhaustively go through every one of these functions.
Instead, we mostly focus on the high-level guidelines for building different layers using these key functions.
Below we start introducing each layer in details, starting from the required ones.

[^1]: <https://ggplot2.tidyverse.org/reference/index.html>

# Required Layers: Data, Geometries, and Aesthetics Mappings

## Creating Visualizaiton using Data, Geometries, and Aesthetics Mappings
...
Geometry layer specify what type of visualization to use, e.g., scatterplot, barplot, and boxplot.
We use `+ geom_xxx()` to select the geometry layer where the function name specifies the visualization type.
For example `geom_point()` is the scatterplot and `geom_line()` is the line plot.
The `+` is used to connect the functions setting up each layer.

Different visualization types come with different visual elements and hence have different lists of aesthetic dimensions.
For example, the scatterplot has x/y-axis position, symbol color, symbol size, symbol shape, etc.
The line plot has has x/y-axis position, line color, line type (solid, dash, or dotted), line width, etc.
We need to specify what variables in the data layer are mapped to which aesthetic dimensions in the geometry layer.
...
| Aesthetic Dimension to be Mapped | Description                                                         |
|--------------------|----------------------------------------------------|
| `x=`                             | x axis position                                                     |
| `y=`                             | y axis position                                                     |
| `fill=`                          | fill color                                                          |
...
It usually displays values for two variables with one variable represented by the x-coordinate and the other by the y-coordinate.
The points can be plotted in different color/shape/size so that additional variables can be displayed.

To generate this scatter plot, we need to specify these three layers in ggplot.
First, we specify the data layer using `ggplot(data=college)`.
...
We would expect the higher SAT corresponds to the lower admission rate, so this visualization presents a somewhat surprising pattern, which is mostly due to the universities with low average SAT scores.
...
So far, we have used two aesthetic dimensions (x and y axes), and there are many other unused dimensions, such as color, shape, size, and etc.
...
Suppose we are interested in the association among `sat_avg`, `admission_rate`, `undergrads` (number of students), and `control` (public or private university).
...
The bubble plot shows that the negative relationship between SAT and admission rate is stronger than the positive relationship, because most of the large bubbles display the negative trend, whereas only a few small bubbles show the positive trend.
...
We also see an odd observation in the figure.
...
We may later find out what that university is by adding a label.
...
Below are two figures comparing different aesthetic dimensions, Figures \@ref(fig:contvar) and \@ref(fig:discvar).
...
Area is mostly used to the same sign values (all positive or all negative).
...
When visualizing a few variables, we need to first decide which sets variables are more important in the visualization because they should be mapped the more powerful aesthetic dimensions.
The remaining variables can be mapped to the remaining aesthetic mappings.
This choice depends on the research question.
For example, if we are mostly interested in the association between SAT and admission rate, and we are also interested in how undergrads, region, and control, affect this relationship, then we would have the following mapping.
...
The complexity of the visualization often hurts the interpretability, making the visualization hard to understand.
...
To solve that, we can use a ring to represent the bubble/university with the interior filled with color, We achieve this by specifying the aesthetic dimension shape to `shape = 21`.
...
We set `fill = control` inside the `aes()` function to specify the interior color.
We set `color = "black"` outside the `aes()` to draw black rings.
...
The differences among these lines are where the data layer and the aesthetic mapping appear, in the geometry layer `geom_point()` or in `ggplot()`.
...
In face, the syntax of `ggplot` allows us to treat each layer as an object for easy modification.
...
In addition, we can put multiple geometries in the same visualization using `+`.
Note that We can also put multiple visualizations in the same canvas using `grid.arrange()` in the R package `grid` and `gridExtra`.

When we create an object in R to contain visualization, the visualization does not get printed to the screen.
Instead, when typing the name of the object and hitting enter, the content of the object, i.e., the visualization, will be printed.
...
Geometries specify the types of visualizations we want, such as scatter plot, histogram, bar plot, pie chart, and etc.
So far, we have used the scatter plot as a running example, but there are many other choices of geometries.
Depending on the number of variables (one or more) and the types of variables (quantitative/quality), we can plot different visualizations.
...
The complete list of all the geometries in `ggplot2` is available at its official website[^2].
...
Often times, statistics are the same as geometries.\
...
If we further want to obtain a functional relationship between these two variables, then we can add a nonlinear regression line which is nothing but a curve that mimics the trend in data.
...
# Scales and Coordinates Layers

Next we discuss the scale and coordinate.
The scale layer sets up various properties of each aesthetic dimensions, such as the range of x-axis, transformation on the y-axis, the color choices, and range of symbol size, etc.
In the college data example, what if we would like make customized labels, text, and ticks, or transform certain variables?
What if we would like to focus on a certain area of the figure by setting the range of x- and y-axes?
This type of customization is done by using the scale functions.
...
Previously, we have introduced the concept of geometry, i.e., visualization type, and explain that each geometry corresponds to its own set of aesthetic dimensions.
For example, the scatterplot has x-/y-axes, color, size, shape of the symbol, etc.
Meanwhile, the line plot has x-/y-axes, color, width, type of the line, etc.
Using the aesthetic mapping, we can decide which variables are mapped to which aesthetic dimensions.
After the mapping, we often need to configure these aesthetic dimensions, which is the objective of the scale layer.
...
Because different visualization types correspond to different aesthetic dimensions dimensions, we use different functions within the scale layer based on the specific circumstances.
...
The complete list of scale layer functions can be found on the ggplot official website in the scale functions list[^3].
...
**Axis Range, Text, Ticks, Labels, and Colors:** For the bubble chart depicting the relationship between SAT scores and admission rates that we discussed earlier, how can we convey more information through configuring these aesthetic dimensions?
...
We will address the configuration of the color aesthetic dimension separately in a subsequent section.
...
**Transformation:** Sometimes, original variables in the data set may not be suitable for direct visualization, such as when their distribution is highly concentrated.
...
If we want to analyze the relationship between brain weight and body weight for different animals, a scatterplot is a natural choice.

In the code below, we first create a scatterplot.
However, it is challenging to see the data pattern from the scatterplot because most data points are concentrated in a small area.
...
They have slight differences, and readers are encouraged to try them out.
...
We can see that on the lower left corner, we have mouse and bat.
...
For example, we plot five red points whose body weight are 50, 100, 150, 200, 250, i.e., equally spaced.
...
We can then map this new variable to the color dimension of the graph, achieving the goal of using color for comparison (left graph).
...
Alternatively, if we want to display the admission rates for all these private schools, we can directly map the admission rate variable to the color dimension of the visualization, accomplishing the goal of using color for comparison (right graph).
We can see that as the SAT score increases, tuition also become more expensive, while admission rates gradually decrease.
This phenomenon is understandable.
As the schools become more selective, the admitted students naturally have better academic performance.
At the same time, these schools may charge higher tuition to provide a more comprehensive education.
...
Due to space constraints, we display only a small random sample of colors below.
...
For example, (255, 255, 255) represents white because it has highest possible proportion of red, green, and blue.
...
Sometimes, the integer between 0 and 255 can be replaced by a numerical value between 0 and 1 where 1 corresponds to 255 and 0 corresponds to 0.
...
-   Sequential color palettes: Used for visualizing continuous variables.
    Colors in these schemes transition gradually, making them suitable for representing the increase or decrease in continuous variables.
    Schemes: Colors transition smoothly, making them suitable for visualizing continuous variables with a gradual change
...
-   Diverging color palettes: Used for visualizing continuous variables with both positive and negative values, such as correlation coefficients or profit/loss.
    ...
    Colors transition from one hue to another, often passing through a neutral color, making them suitable for visualizing continuous variables with both positive and negative values.

-   Discrete color palettes: Used for visualizing discrete variables.
    Colors in these schemes are distinct from each value in the discrete variable, aiding in the effective differentiation of discrete variable values.
    Colors are distinct and have a clear separation, making them suitable for visualizing discrete variables.
...
Let us take the `RColorBrewer` package as an example to introduce them.
The color palettes in `RColorBrewer` are again grouped into three categories: sequential color palettes, diverging color palettes, discrete color palettes.

It's worth noting that the choice of color scheme depends on the nature of the data and the goals of the visualization.
...
Setting a custom palette requires specifying the two endpoint colors, and the palette will automatically generate a series of colors transitioning from the first endpoint color to the second.
Specifically, we can use `scale_color_gradient(low = color1, high = color2)` to use a custom gradient palette, where `color1` and `color2` are the two endpoint colors.
...
Returning to the example of the college dataset, we are now visualizing the admission rates and SAT scores of universities in four different cities (New York, Chicago, Los Angeles, Cincinnati) using colors.
Since the city names represent a discrete variable, we need to use a categorical color palette.
...
The same practice can be used on other type of visualizations too.
...
g4 = cor_mat %>%
  ...
  scale_fill_gradientn(name = "Tuition", colors = c("blue", "white", "red")) +
  theme(legend.position = "top")
grid.arrange(g1, g2, g3, g4, ncol=2)
```
...
This visualization is motivated by the award-winning [visualization of national park visitors](https://jordan-vincent.com/night-under-the-stars/).
...
Here is another example of sunspots data set.
...
We first visualize the data using the line plot.
...
For the college data, suppose we would like to know the relationship between the SAT and admission rate conditional on a third variable, what should we do?
...
-   `facet_grid()` generates a table/grid of plots. For splitting the data by **two or more** qualitative variables, `facet_grid()` is best.
...
Since we use to variables to create the facets, we use `facet_grid()` below.
As we can see, the tuition is more expensive for private schools than public schools.
Meanwhile, for private schools, the relationship between tuition and SAT shows a positive trend.
...
In this case, we visualize four variables at the same time.
...
**Examining Simpson's Paradox using Facet Layer:** Faceting (or stratification) is an important tool for identifying Simpson's paradox.
...
The complete data is provided in the R `lahman` package available (<https://cran.r-project.org/web/packages/Lahman/>).
...
Usually, more runs lead to more wins and fewer losses.
However, when we visualize the data, we actually get positive slope, which is puzzling.
...
If we plot the runs vs the number of games, and losses vs the number of games, here is what we found.
As the years go by, there are more games in total, hence, more runs and more losses.
In other words, the number of games is the confounder, which leads to a positive correlation between run and loss.
...
Below we separate the data into different group according to the number of games.
We divide the number of games into 9 bins (0-20, 21-40, ..., 140+), and fit separate regression within each **stratum or facets**.
Below are the results.
We mostly observe negative estimated slopes in each strata, excep the first stratum where the sample size is small.
...
This is essentially because of the confounder, which is the number of games in this case.
...
For example, Here is a list of templates in the theme layer.
...
# Scatterplot and Overplotting

We have used the scatterplot to demonstrate the usage of grammar of graphics.
In this section, we further discuss some of the additional topics in the scatterplot.
One of frequently encountered issues in scatterplot is overplotting.
...
When we take the average across all universities in each state, the variation in the SAT score for each state becomes even less.
...
1.  In the college data set, suppose we are interested in the relationship between tuition and loan default rate, please use the scatterplot to visualize such a relationship and interpret the results.
...
2.  Based on the scatterplot above, please try following
